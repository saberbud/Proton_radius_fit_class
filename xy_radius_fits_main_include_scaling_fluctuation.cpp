...
int main(Int_t argc, char *argv[])
{
  ...
  */
  ...
  npara=2;
  parain[0]=0.44e-3;parain[1]=9.7e-6;
  Rfc.set_para(parain, npara);
  Rfc.GE_fit(8);
  Rfc.get_fit_result(fit_result);
  cout << "R= " << fit_result[0] << " , Rerr= " << fit_result[1] << " , chi2= " << fit_result[2] << endl;
  ...
  int id_gen,id_nois;
  id_gen=3;  //1: dipole, 2: monopole 3: Gaussian with float norm
  ...
  for(int k=0;k<nrp;k++){
    ...
    */
    ...
    for(int j=0;j<nfits;j++){
      ...
      npara=2;
      // parain[0]=0.44e-3;parain[1]=9.7e-6;  //DH 1
      // parain[0]=1.14e-3;parain[1]=6.8e-5;  //DH 2
      parain[0]=0.93e-3;parain[1]=3.9e-5;  //DH 3
      Rfc.set_para(parain, npara);
      Rfc.GE_fit(id_fit[j]);
